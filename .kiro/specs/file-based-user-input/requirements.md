...
**User Story:** Как пользователь, я хочу, чтобы процесс приостанавливался после открытия файла с интерактивным меню выбора, чтобы удобно управлять процессом без ввода текста.

#### Acceptance Criteria

1. WHEN Template_File открыт в Editor THEN THE Workflow_Orchestrator SHALL приостановить выполнение процесса
2. WHEN процесс приостановлен THEN THE Workflow_Orchestrator SHALL отобразить интерактивное меню с вариантами действий
3. THE Workflow_Orchestrator SHALL предложить выбор стрелочками между вариантами: "Продолжить", "Отложить"
4. THE вариант "Продолжить" SHALL быть выбран по умолчанию
5. WHEN пользователь нажимает Enter без изменения выбора THEN THE Workflow_Orchestrator SHALL выполнить действие по умолчанию (продолжить)
6. WHEN пользователь выбирает "Продолжить" и нажимает Enter THEN THE Workflow_Orchestrator SHALL прочитать заполненный файл и продолжить выполнение
7. WHEN пользователь выбирает "Отложить" и нажимает Enter THEN THE Workflow_Orchestrator SHALL сохранить состояние и завершить выполнение для последующего возобновления
