...
> **Важно:** В примерах ниже используется команда `workflow-orchestrator`. Если вы не делали `npm link`, замените её на `node dist/cli/cli.js`.

## Предварительные требования

1. Установлен Node.js 18+ и npm
2. Установлен хотя бы один CLI-адаптер (например, claude-cli)
3. CLI-адаптеры авторизованы (например, `claude auth login`)

> **Примечание:** API ключи нужны только для OpenAI-compatible адаптеров. CLI-утилиты используют собственную авторизацию.
